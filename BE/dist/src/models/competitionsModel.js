...
const inputDataCompetitions = async (RegistrationID, newDataCompetitions) => {
    ...
    try {
        ...
        if (Pernyataan_Origalitas !== "") {
            await connection.query(`UPDATE Competitions SET Pernyataan_Origalitas = ? WHERE RegistrationID = ?`, [Pernyataan_Origalitas, RegistrationID]);
            console.log("Update Pernyataan_Originalitas:", Pernyataan_Origalitas);
        }
        if (Proposal !== "") {
            await connection.query(`UPDATE Competitions SET Proposal = ? WHERE RegistrationID = ?`, [Proposal, RegistrationID]);
            console.log("Update Proposal:", Proposal);
        }
        if (Dokumen_Substansi !== "") {
            await connection.query(`UPDATE Competitions SET Dokumen_Substansi = ? WHERE RegistrationID = ?`, [Dokumen_Substansi, RegistrationID]);
            console.log("Update Dokumen_Substansi:", Dokumen_Substansi);
        }
        if (title !== "") {
            await connection.query(`UPDATE Competitions SET title = ? WHERE RegistrationID = ?`, [title, RegistrationID]);
