import { getAllCompetitions, inputDataCompetitions, checkCompetitionsByRegistrationID, } from "../models/competitionsModel";
...
export const inputDataCompetitionsController = async (req, res) => {
    try {
        ...
        const status_cek = await checkStatusRegistrasiWithExpectedStatus(RegistrationID, 2);
        if (status_cek !== 2) {
            return res
                .status(400)
        ...
        const proposalUrl = await uploadFile(file.Proposal[0].buffer, "Proposal");
        const dokumenSubstansiUrl = await uploadFile(file.Dokumen_Substansi[0].buffer, "Dokumen_Substansi");
        const pernyataanOriginalitasUrl = await uploadFile(file.Pernyataan_Originalitas[0].buffer, "Pernyataan_Originalitas");
        ...
        const result = await inputDataCompetitions(RegistrationID, newDataCompetitions);
        if (result !== 200) {
...
export const checkCompetitionsController = async (req, res) => {
    try {
        const authHeader = req.headers.authorization;
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
            return res.status(401).json({
        ...
        const RegistrationID = decoded.RegistrationID;
        const status_cek = await checkCompetitionsByRegistrationID(RegistrationID);
        if (status_cek !== 1) {
            return res.status(400).json({
